// Libraries -----------------------------------------------------------------------------------------------------------
#define BLYNK_TEMPLATE_ID "TMPLUCUPue9V"
#define BLYNK_DEVICE_NAME "FULL CONT SHEKHAN"
#define BLYNK_AUTH_TOKEN "xR0XyB29x406Ux9mr1MuHkcZ2L4V7eU2"
...
const char* serverName = "http://emdpu.shaxirash.com/post_data_shekhan.php";
String apiKeyValue = "tPmAT5Ab3j7F9";


// WiFi parameters ---------------------------------------------------------------------------------------------------
#define WLAN_SSID       "IT Department"
#define WLAN_PASS       "itTI!2022"
...
// define split and computer status pin in blynk 
WidgetLED split_status(V7);
WidgetLED com_status(V9);
 
// define split and computer pin in esp32 
#define device1 15
#define device2 21
...
// computer and light_On -----------------------------------------
void Device2_On()
{
 digitalWrite(device2,HIGH); 
 tft.fillRect(36, 80, 80, 10, ST7735_RED);
 tft.fillRect(36, 89, 89, 10, ST7735_RED);
 tft.setCursor(7, 80); tft.print("COMPU: ON");
 tft.setCursor(7, 89); tft.print("FLAG: 1");
 info.flag1=1;
 save_to_eeprom();
 com_status.on();
}

// computer and light_Off ---------------------------------------------
void Device2_Off()
{
 digitalWrite(device2,LOW);  
 tft.fillRect(36, 80, 80, 10, ST7735_RED);
 tft.fillRect(36, 90, 90, 10, ST7735_RED);
 tft.setCursor(7, 80); tft.print("COMPU: OFF");
 tft.setCursor(7, 89); tft.print("FLAG: 0");
 info.flag1=0;
 save_to_eeprom();
 com_status.off();
}
...
void Device1_On()
{
 ...
 tft.fillRect(40, 102, 102, 10, ST7735_RED);
 tft.fillRect(40, 112, 112, 10, ST7735_RED);
 tft.setCursor(7, 102); tft.print("SPLIT: ON");
 tft.setCursor(7, 112); tft.print("FLAG: 1");
 info.flag=1;
 save_to_eeprom();
 split_status.on();
}
...
void Device1_Off()
{
 ...
 tft.fillRect(40, 102, 102, 10, ST7735_RED);
 tft.fillRect(40, 112, 112, 10, ST7735_RED);
 tft.setCursor(7, 102); tft.print("SPLIT: OFF");
 tft.setCursor(7, 112); tft.print("FLAG: 0");
 info.flag=0;
 save_to_eeprom();
 split_status.off();
}
...
void publish_em()
{
 //define clint and http protocol connection
    ...
    String httpRequestData = "api_key=" + apiKeyValue + "&voltage=" + float(meas.voltage)
                           + "&current=" + float(meas.current) + "&power=" + float(meas.power) + "&frequency=" + float(meas.frequency) + "&temp=" + float(meas.temp) + "&hum=" + float(meas.hum) + "&device_no=" + int(device_no) + "&flag=" + int(info.flag) + "&flag1=" + int(info.flag1) +  "&energy=" + int(engy) + "&pof=" + int(pof) + "";
...
//stracture type of auto control the split devise
if (info.flag == 1){
 if(meas.temp_threshold>=24)
 {
  if(meas.temp<=meas.temp_threshold-2)
   Device1_On();
  else if (meas.temp>=meas.temp_threshold)
   Device1_Off();
 }
 else if(meas.temp_threshold<24)
 {
  if(meas.temp<=meas.temp_threshold)
   Device1_Off();
  else if (meas.temp>=meas.temp_threshold+2)
   Device1_On();
 }
...
//get status of computer and light devise from blynk
BLYNK_WRITE(V10)  //computer and light ON OFF from Blynk app
{
 int w_pinValue2;
 w_pinValue2 = param.asInt();
 if (w_pinValue2 == 1)
  Device2_On();
 else Device2_Off();
}
...
// starting read sensor of temp and hum
 ...
 // define computer and light display on secreen 
...
//check split status
 if(info.flag==1) Device1_On();
 else Device1_Off();
 
//check computer and light status
 if(info.flag1==1) Device2_On();
 else Device2_Off();
}

//read split temprature value that set from user status
BLYNK_WRITE(V8)
{
 meas.temp_threshold = param.asInt();
 save_to_eeprom();
 read_from_eeprom();
}
